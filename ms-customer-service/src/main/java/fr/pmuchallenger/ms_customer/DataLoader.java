...
import fr.pmuchallenger.ms_commande.model.dto.request.AddCustomerRequest;
import fr.pmuchallenger.ms_commande.model.dto.request.PartantAddRequest;
import fr.pmuchallenger.ms_commande.services.CustomerService;
...
public class DataLoader implements CommandLineRunner {
    private final CustomerService courseService;

    @Autowired
    public DataLoader(CustomerService courseService) {
    ...
    public void run(String... args) throws Exception {
        ...
        AddCustomerRequest courseRequest = AddCustomerRequest.builder().date(LocalDate.of(2023, 1, 29)).numero(1).nom("prix d'Amérique").partant(buildPartants()).build();
        ...
        if (isUnique) {
            ...
            System.out.println("Customer créée avec succès.");
        } else {
